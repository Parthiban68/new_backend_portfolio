...
//               >
//                 <Icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110`}   color={tech.color}
// />
//               </motion.button>
...
//                 {hoveredIdx === idx && (
//                   <motion.div
//                     // FIX: Handle X translation inside Framer Motion, not Tailwind
//                     initial={{ opacity: 0, y: 8, x: "-50%", scale: 0.95 }}
//                     animate={{ opacity: 1, y: 0, x: "-50%", scale: 1 }}
//                     exit={{ opacity: 0, y: 4, x: "-50%", scale: 0.95 }}
//                     transition={{ duration: 0.15 }}
//                     // Removed -translate-x-1/2, added origin-bottom and flex-col
//                     className="absolute bottom-full mb-3 left-1/2 z-50 pointer-events-none whitespace-nowrap origin-bottom flex flex-col items-center"
//                   >
...
//                     </div>
//                     {/* Tooltip Arrow */}
//                     <div className="w-2 h-2 bg-slate-900/90 rotate-45 -mt-1 border-r border-b border-slate-700/50" />
//                   </motion.div>
//                 )}
//               </AnimatePresence>
//             </div>
...
const TechStackBar = () => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 pt-2">
      {techStackData.map((tech, idx) => {
        const Icon = tech.icon;
        const isDarkAdapted = tech.name === "GitHub" || tech.name === "Next.js";
        ...
        return (
          ...
          >
            <motion.button
              whileHover={{ y: -4, scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="w-11 h-11 rounded-2xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-slate-200/80 dark:border-slate-700/80 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)] flex items-center justify-center transition-all duration-300 hover:border-blue-300 dark:hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 dark:hover:shadow-blue-500/20 group"
            >
              <Icon 
                className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${isDarkAdapted ? 'text-black dark:text-white' : ''}`}   
            ...
            <AnimatePresence>
              {hoveredIdx === idx && (
                <motion.div
                  ...
                  className="absolute bottom-full mb-3 left-1/2 z-50 pointer-events-none whitespace-nowrap origin-bottom flex flex-col items-center"
                >
                  <div className="bg-slate-900/90 dark:bg-slate-100/90 text-white dark:text-slate-900 text-xs px-3 py-1.5 rounded-xl shadow-xl backdrop-blur-md border border-slate-700/50 dark:border-slate-300/50 flex flex-col items-center gap-0.5">
                    <span className="font-semibold text-slate-100 dark:text-slate-900">{tech.name}</span>
                    <span className="text-[10px] text-slate-400 dark:text-slate-600 font-normal">{tech.desc}</span>
                  </div>
                  {/* Tooltip Arrow */}
                  <div className="w-2 h-2 bg-slate-900/90 dark:bg-slate-100/90 rotate-45 -mt-1 border-r border-b border-slate-700/50 dark:border-slate-300/50" />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
export default function AboutHero() {
  ...
  return (
    <section className="relative w-full min-h-screen bg-[#fcfcfd] dark:bg-neutral-950 text-slate-900 dark:text-slate-100 overflow-hidden flex flex-col justify-between selection:bg-blue-500 selection:text-white transition-colors duration-500">
      ...
      <div className="absolute bottom-20 right-10 w-[400px] h-[400px] bg-indigo-500/10 dark:bg-indigo-600/10 rounded-full blur-[130px] pointer-events-none" />

      {/* HERO MAIN CONTAINER */}
      <div className="max-w-[1400px] w-full mx-auto px-6 sm:px-8 lg:px-12 pt-12 pb-16 relative z-10 flex-grow flex items-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center w-full">
          ...
          <div className="lg:col-span-7 flex flex-col justify-center space-y-8">
            ...
            <div className="space-y-3">
              <motion.span 
                ...
                animate={{ opacity: 1 }}
                transition={{ duration: 0.5, delay: 0.1 }}
                className="text-xs font-mono tracking-widest text-blue-600 dark:text-blue-400 uppercase font-bold"
              >
                # About Me
              </motion.span>
              ...
              <motion.h1 
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.7, delay: 0.15 }}
                className="text-4xl sm:text-5xl xl:text-6xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-[1.08]"
              >
                I'm <span className="text-slate-900 dark:text-white">Parthiban</span>
              </motion.h1>
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.45 }}
              className="inline-flex flex-wrap items-center gap-3 sm:gap-4 text-xs font-medium text-slate-600 dark:text-slate-300 bg-white/70 dark:bg-slate-800/70 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 shadow-sm max-w-max"
            >
              <div className="flex items-center gap-1.5">
                <MapPin className="w-3.5 h-3.5 text-red-500" />
                <span>Coimbatore, Tamil Nadu, India</span>
              ...
              <div className="flex items-center gap-1.5 font-mono">
                <Clock className="w-3.5 h-3.5 text-blue-500" />
                <span className="text-slate-800 dark:text-slate-100 font-semibold">{timeStr || '12:00:00 PM'} IST</span>
              </div>
              <span className="text-slate-300 dark:text-slate-600">•</span>
              <div className="flex items-center gap-1.5">
                <Zap className="w-3.5 h-3.5 text-amber-500" />
          ...
          <div className="lg:col-span-5 relative flex items-center justify-center min-h-[520px]">
            ...
            <motion.div 
              animate={{ y: [0, -10, 0], rotate: [0, 1, 0] }}
              transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
              className="absolute -top-4 right-2 sm:right-6 z-20 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-700/80 p-3.5 rounded-2xl shadow-xl shadow-slate-900/5 dark:shadow-[0_8px_30px_rgba(0,0,0,0.4)] flex items-center gap-3"
            >
              ...
              <div>
                <div className="text-xs font-bold text-slate-800 dark:text-slate-200">Multi Tenant</div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">Isolated Schemas</div>
              </div>
            </motion.div>
            ...
            <motion.div 
              animate={{ y: [0, 10, 0], rotate: [0, -1, 0] }}
              transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }}
              className="absolute -bottom-6 left-2 sm:left-4 z-20 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-700/80 p-3.5 rounded-2xl shadow-xl shadow-slate-900/5 dark:shadow-[0_8px_30px_rgba(0,0,0,0.4)] flex items-center gap-3"
            >
              ...
              <div>
                <div className="text-xs font-bold text-slate-800 dark:text-slate-200">Auth Infrastructure</div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">JWT / RBAC / Sessions</div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              whileHover={{ rotate: 0, scale: 1.02 }}
              transition={{ duration: 0.6 }}
              className="relative z-10 w-full max-w-[340px] bg-white dark:bg-slate-800 p-4 pb-6 rounded-3xl border border-slate-200/80 dark:border-slate-700/80 shadow-[0_20px_50px_rgba(0,0,0,0.08)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500 group"
            >
              {/* Image Container with Framing */}
